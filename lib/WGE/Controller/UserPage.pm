...
sub my_bookmarks :Path('/my_bookmarks'){
    ...
    my $shared_designs_rs = $c->user->shared_designs;
    ...
    $bookmarks->{'Human'}->{shared_designs} = [map { $_->as_hash } $shared_designs_rs->search({ species_id => 'Human' }, { order_by => 'created_at DESC' }) ];
    ...
    $bookmarks->{'Mouse'}->{shared_designs} = [map { $_->as_hash } $shared_designs_rs->search({ species_id => 'Mouse' }, { order_by => 'created_at DESC' }) ];
